...
- 응답 보장:
  - `muscle_code`: 항상 `^[a-z0-9_]+$`
  - `display_name_ko`: placeholder 미반환
  - `muscle` 필드는 하위 호환용으로 유지
...
6. 히트맵 응답의 `muscle_code`를 SVG path 키로 사용 (`muscle`은 하위 호환 필드)
7. 미응답 근육을 별도 계산하지 말고, RPC 결과 그대로 사용
8. 서버 시간 기준(`now()`)으로 색상이 계산되므로 클라이언트에서 시간 계산 금지
9. 기록 저장 시 payload 규칙:
...
10. `display_name_ko`가 비어있거나 placeholder이면 프론트 fallback으로 대체하지 말고 오류 이벤트로 수집
